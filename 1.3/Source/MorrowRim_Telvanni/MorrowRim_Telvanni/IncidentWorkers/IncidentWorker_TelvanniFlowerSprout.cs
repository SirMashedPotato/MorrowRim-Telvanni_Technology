...
namespace MorrowRim_Telvanni
{
    class IncidentWorker_TelvanniFlowerSprout : IncidentWorker
    {
		...
		{
			if (!c.Standable(map) || c.Fogged(map) || map.fertilityGrid.FertilityAt(c) < ThingDefOf.Telvanni_AshCap.plant.fertilityMin || !c.GetRoom(map).PsychologicallyOutdoors || c.GetEdifice(map) != null || !PlantUtility.GrowthSeasonNow(c, map, false))
			{
				return false;
			}
