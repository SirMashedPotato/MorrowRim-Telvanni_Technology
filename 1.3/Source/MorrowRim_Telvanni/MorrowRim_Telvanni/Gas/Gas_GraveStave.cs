                    ...
                    if (hashSet != null)
                    {
                        foreach (Thing thing in hashSet)
                        {
                            ...
                            if (thing != null && thing is Pawn)
                            {
                                ...
                                if (p.health.hediffSet.GetFirstHediffOfDef(HediffDefOf.MorrowRim_ElixirActiveGrave_High) != null)
                                {
                                    return;
                                }
                                float num = 0.028758334f;
                                //num *= p.GetStatValue(StatDefOf., true);
                                if (num != 0f && !this.Destroyed)
                                {
                                    float num2 = Mathf.Lerp(0.85f, 1.15f, Rand.ValueSeeded(p.thingIDNumber ^ 74374237));
                                    num *= num2;
                                    HealthUtility.AdjustSeverity(p, HediffDefOf.MorrowRim_ArmorCorrosion, num);
                                }
                            }
                        }
                    }
