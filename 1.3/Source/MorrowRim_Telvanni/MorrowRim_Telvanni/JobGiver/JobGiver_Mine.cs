...
namespace MorrowRim_Telvanni
{
	class JobGiver_Mine : ThinkNode_JobGiver
	{
		...
		{
			foreach (Designation designation in pawn.Map.designationManager.SpawnedDesignationsOfDef(RimWorld.DesignationDefOf.Mine))
			{
				bool flag = false;
				for (int i = 0; i < 8; i++)
				{
		...
		public Job JobOnThing(Pawn pawn, Thing t, bool forced = false)
		{
			...
			if (pawn.Map.designationManager.DesignationAt(t.Position, RimWorld.DesignationDefOf.Mine) == null || !ForbidUtility.InAllowedArea(t.Position, pawn))
			{
				return null;
			}
