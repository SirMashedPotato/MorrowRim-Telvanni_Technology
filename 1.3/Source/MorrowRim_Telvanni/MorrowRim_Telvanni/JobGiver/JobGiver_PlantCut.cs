	...
	class JobGiver_PlantCut : ThinkNode_JobGiver
	{
		...
		public IEnumerable<Thing> PotentialWorkThingsGlobal(Pawn pawn)
		{
			...
			for (int i = 0; i < desList.Count; i = num + 1)
			{
				...
				if (designation.def == RimWorld.DesignationDefOf.CutPlant || designation.def == RimWorld.DesignationDefOf.HarvestPlant)
				{
		...
		public Job JobOnThing(Pawn pawn, Thing t, bool forced = false)
		{
			...
			{
				if (designation.def == RimWorld.DesignationDefOf.HarvestPlant)
				{
				...
				if (designation.def == RimWorld.DesignationDefOf.CutPlant)
				{
