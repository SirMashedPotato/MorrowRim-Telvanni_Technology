...
namespace MorrowRim_Telvanni
{
    class IncidentWorker_TelvanniExtractSprout : IncidentWorker
	{
		...
		{
			return CellFinderLoose.TryFindRandomNotEdgeCellWith(10, (IntVec3 x) => this.CanSpawnAt(x, map, extractPlant) && x.GetRoom(map, RegionType.Set_Passable).CellCount >= MinRoomCells, map, out cell);
		}

		private bool CanSpawnAt(IntVec3 c, Map map, ThingDef extractPlant)
		{
			if (!c.Standable(map) || c.Fogged(map) || map.fertilityGrid.FertilityAt(c) < extractPlant.plant.fertilityMin || !c.GetRoom(map, RegionType.Set_Passable).PsychologicallyOutdoors || c.GetEdifice(map) != null || !PlantUtility.GrowthSeasonNow(c, map, false))
			{
				return false;
			}
